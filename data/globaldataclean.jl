...
using CSV, DataFrames, JLD2, HypothesisTests, Plots, TensorToolbox

# function for difference of a country's data with optional log transformation
function cleancountry(country::String, df::DataFrame; logtransform::Bool=false)
...
for (i, country) in enumerate(countries)
    matdata[2, i, :] = cleancountry(country, gdp)[20:115]
end

# production with log difference
for (i, country) in enumerate(countries)
    matdata[3, i, :] = cleancountry(country, prod)[20:115]
end
